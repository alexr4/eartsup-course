...
void setup()
{
  ...
  mesPoints.add(new Point(0, (3*PI)/2, width/2, height-200, 0));
}
    ...
    if (cycle < limiteCycle)
    {
      if (pts.vie == false && pts.exploded == false)
      {
        for (int j=0; j<nbNewPts; j++)
        {
          ...
          mesPoints.add(new Point(0, angle, pts.x, pts.y, 0));
        }
...
void mousePressed()
{
  ...
  mesPoints.add(new Point(0, (3*PI)/2, width/2, height-20, 0));
}
