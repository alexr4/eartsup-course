...
void setup()
{
  ...
  mesPoints.add(new Point(0, (3*PI)/2, width/2, height/2, life));
}
  ...
  for (int i=0; i<mesPoints.size(); i++)
  {
    ...
    if (cycle < limiteCycle)
    {
      if (pts.vie == false && pts.exploded == false)
      {
        ...
        {
          float angle = map(j, 0, nbNewPts, 0, PI);
          mesPoints.add(new Point(0, pts.theta+inc+angle, pts.x, pts.y, int(pts.lifeTime-100)));
        }
